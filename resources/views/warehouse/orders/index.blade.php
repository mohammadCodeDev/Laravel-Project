                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                            <form method="POST" action="{{ route('warehouse.orders.deliver', $order) }}">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="text-blue-600 hover:text-blue-900">{{ __('Mark as Delivered') }}</button>
                                            </form>
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="4" class="px-6 py-4 text-center">{{ __('No confirmed orders to display.') }}</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
